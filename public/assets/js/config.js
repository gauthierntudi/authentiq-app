!(function () {
    var t = sessionStorage.getItem("__AUTHENTIQ_CONFIG__"),
        e = document.getElementsByTagName("html")[0],
        i = { theme: "dark", nav: "vertical", layout: { mode: "fluid" }, topbar: { color: "dark" }, menu: { color: "dark" }, sidenav: { size: "sm-hover-active", user: !1 } },
    ...
    if (((config.menu.color = null !== n ? n : i.menu.color), (window.defaultConfig = JSON.parse(JSON.stringify(config))), null !== t && (config = JSON.parse(t)), (window.config = config))) {
        if ("vertical" == config.nav) {
            ...
            window.innerWidth <= 767 ? (t = "full") : 767 <= window.innerWidth && window.innerWidth <= 1140 && "full" !== self.config.sidenav.size && "condensed" !== self.config.sidenav.size && (t = "condensed"),
